...
- **编辑与 Manage**：对资产的 **Edit**、**Manage**（含各类**状态转移**：上线/下线/Draft、FG 的 Online 相关操作等**写配置或改状态**）要求操作者在**该资产绑定的 Biz Team** 下具备 **Editor** 或 **Admin**；不满足则入口置灰或后端拒绝。仅读展示、导出文档、复制只读链接等不在此限（仍需具备平台登录与 Viewer 等基础角色）。
...
**职责**：与 §1.2.3、架构 §4.3 / §4.5 一致；绑定 FS + Transformer，配置 Offline/Online FG Config，同一时刻仅一版生效；产品侧提供 FG **Versions**（配置快照）用于审计；Serving 画布与 Manage 状态机见 §3.3.0 / §3.3.A。**Biz Team 与列表可见性**见 **§2.3**（Viewer 全量可见；**Manage**、配置 **Edit** 等须对应 Biz Team 的 Editor/Admin）。
...
- **Publish**：**仅 Test Run 通过后** **Publish** 才可点击；点击后回到 **FG 详情页**；若 FG 为 **Online** → 状态变为 **Online Changing**；**右上角 Toast** 提示 Serving Config 已更新，**仍需手动 Manage > Online** 后线上才实际生效。
...
**权威状态转移**以下列设计稿为准（与下文文字、Mermaid 互证）：

![Feature Group 状态机](../assets/fg-feature-group-status-machine.png)

**转移条件摘要**（与上图一致）：

- **进入 Draft**：**Add FG**；Draft 上 **edit & save** 仍停留在 Draft。
- **Draft → Online**：**Training Config 与 Serving Config 均已配置** 后，方可 **Manage > Online**（二者任一为空则 **不可** 操作 Online）。
- **Draft → Deleted**：**Delete**（软删）。
- **Online → Online Changing**：（1）用户编辑 **Training 或 Serving Config** 并保存；（2）**上游自动检测**：Serving 依赖的 **Feature Source / Transformation** 版本变更。**Basic Info 变更直接生效，不进入 Online Changing**。
- **Online Changing**：**不可 Edit**（配置编辑入口置灰或等价限制）；须由 **Admin** 执行 **Manage > Online**（Config Diff 确认后 **覆盖线上版本**）回到 **Online**；上游触发场景另向 **FG Owner** 推送 **Alert**，**不自动改 FG**，由 Admin 手动推进版本。
- **Online → Draft**：**Manage > Draft**（**需 Admin**，二次确认；下游影响见实现/mock）。

下列 Mermaid 为上图的结构化摘要；**红色箭头（需 Admin）** 在图中标注，下表用「Admin」注明。

```mermaid
stateDiagram-v2
    [*] --> Draft : Add FG
    Draft --> Draft : edit and save
    Draft --> Online : T and S configured\nManage Online
    Draft --> Deleted : delete
    Online --> OnlineChanging : Edit T or S Config\nor upstream FS or Trans change
    Online --> Draft : Manage Draft (Admin)
    OnlineChanging --> Online : Manage Online\noverwrite live (Admin)
    Deleted --> [*]
```

| 状态 | Tag 颜色 | 含义 |
|------|----------|------|
| **Draft** | `tag-gray` | 未上线；Draft 内可反复 edit & save |
| **Online** | `tag-green` | 当前配置对下游生效 |
| **Online Changing** | `tag-orange` | 有待合并的 T/S 或依赖变更；**不可 Edit**；待 **Admin**：**Manage > Online** + **Config Diff** 确认后覆盖线上 |
| **(Deleted)** | — | 软删除，列表不可见 |

**Serving 画布与 Publish**（与 §3.3.A.1 一致）：**Test Run 通过** 后才可 **Publish**；Publish 后回详情，**Online** → **Online Changing**，并 **Toast** 提示须手动 Online 方上线生效。
...
|------|-------|--------|-----------------|
| Sync | Yes | Yes | No（置灰） |
| **Online** | Yes（**仅当 T+S 均已配置**；上线） | — | Yes（**Admin**；Diff 后接受变更、**生成新 FG Version**，见 §3.3.5） |
| **Draft** | — | Yes（**Admin**；二次确认 + 下游依赖说明） | — |
| **Delete** | Yes（软删） | — | — |
...
- **Training**：已配置后卡片 **Edit** 打开 Training Modal；**Weekly / Monthly** 频率见 §3.3.A 英文提示。**Online** 态下保存 Training 变更 → **Online Changing**（Basic 变更不触发，见 §3.3.0）。
- **Serving**：未配置时 Plus 进入 **`/fg/:fgId/serving`**；已配置后可 **Edit**（同路由）进入画布。**Online** 态下 **Publish**（且满足 §3.3.A.1）→ **Online Changing**。
...
- **Training Config Modal**（**仅** 详情 **加号 / Training Edit**）：字段同原 Training 步；**无** Step 导航；**Cancel / Save**；保存后回显 Training 与 Feature List。**Online** FG 保存成功且内容相对线上有变更时 → **Online Changing**。
...
- **Training Config 或 Serving Config**：变更使 FG 进入 **Online Changing**（见 §3.3.0）；**手动 Manage > Online**（Config Diff 确认）后 **生成新 FG Version**。
- **上游依赖变更**：进入 **Online Changing** 并 Alert Owner；**不自动改 FG**；由 **Admin** 手动推进并最终 **Online** 时记入新版本。
...
- **Edit**：打开 Modal 弹窗（标题 "Edit WideTable"），回显 TS 元信息，Owner / Biz Team / Description 可编辑，**WideTable Name** 和 **Region** 置灰只读。**Edit / Add / Delete** 须满足 **§2.3**（该 TS 绑定 Biz Team 的 Editor/Admin）。
