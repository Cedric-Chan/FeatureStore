...
| **Asset Lineage** | 内部平台资产拓扑：FG ↔ FS ↔ Transformer ↔ WideTable 的平台内关系图谱 |
...
- - **Feature Logic Modal**：FG 详情 > Feature List > Action > Logic 触发。三个 Tab：Lineage（全链路血缘 DAG）| Processing（分阶段加工逻辑 + AI 审核状态 | Health（延迟 / 偏移 / 新鲜度信号）。知识由后台 AI Agent T+1 提炼，从外部元数据平台同步，存储在 FS 中。
