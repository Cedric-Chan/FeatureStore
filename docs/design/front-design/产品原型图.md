...
**非范围（后续 PRD）**：详情页 **Serving Config** 与 **Feature Mapping** 的完整分步交互与独立 Modal；本期 Serving 区可保留空占位或简短说明。
...
- **表格**
  ...
  - **Action** 含：**Sync**（原「刷新 Training Config 元数据」；Online Changing / Disabled 时置灰不可点）、**Copy**、**Manage**（下拉菜单，按状态动态展示可用项，见 §3.3.0 矩阵）。**Draft 可点击标题进详情**。
  - 默认分页 **20 条/页**；无行选、无批量。
...
- **Header**：FG 名称 + **Status Tag** + 右侧 **Sync**（英文 tooltip：**Manually refresh latest Training Config metadata**；成功后刷新 **Update Time**）。Online Changing / Disabled 时 Sync 置灰。
- **Manage**：与列表一致（§3.3.0 矩阵）。
- **元信息（Draft 且 Training 未配置时）**：首行 **仅 Basic Info** 卡片（右上角 **Edit** 打开 Basic Modal）；次行 **Training / Serving** 为占位空态，**中间大加号** 打开 **Training Config Modal**；Serving 本期可留空占位。
- **元信息（Training 已配置后）**：Basic + Training 卡片回显；Training 卡片 **Edit** 打开 Training Modal；Serving 占位至后续 PRD；**Feature List** 与离线表字段对齐（无 Serving 配置时 **Serving 列均为 false**）。
- **Tab**：Feature List & Availability、Lineage、Offline DQC、Version History（内容规则不变）。
...
- **全量四步 Wizard**：不再作为默认入口；Serving / Feature Mapping 的独立弹窗与校验见后续 PRD。
- **Test / Submit / Publish**：与线上一致时，在后续「发布」流中承接；本期原型以 **Draft + 分阶段保存** 为主。
