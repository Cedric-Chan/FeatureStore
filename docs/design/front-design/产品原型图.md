...
- **元信息展示**：上半屏三列卡片布局（`fg-info-row`），紧凑工整：
  ...
  - **Training Config**：Data Server（reg_sg_hive / reg_us_hive）；Table Schema / Table Name（下拉，来自平台 Project Table Access List）；Date Partition、Partition Type、Update Frequency（Daily / Weekly / Monthly / ONCE）；Entities Column（多选必填）；Custom Filter（可选）。
  - **Serving Config**：可多组 **Feature Source + Transformation**（Data Latency 来自 Feature Source 元数据，胶囊 Tag；FS/TF 版本 Tag）；Source Type、Input Params 回显；无 Serving 时可留空。
  - **Feature Mapping**（若产品有独立展示）：Serving 输出与 Training 列映射；实时 Compute（SQL 仅引用 Serving 特征名）。
...
- **四步导航条**（Steps Bar）：**① Basic Info → ② Training Config → ③ Serving Config → ④ Feature Mapping**，点击切换步骤，已完成步骤显示主色勾号。
- **表单内容**：
  - **Basic Info**：Region、Module 数据域（默认 External Data）、Owner（只读，自动填充当前用户）。
  - **Training Config**：Data Server；Table Schema / Table Name（Project Table Access 下拉）；Date Partition + Partition Type（同行）；Update Frequency；Entities Column（多选必填）；Custom Filter。
  - **Serving Config**：可增删 **Serving pair**；每对选择 Feature Source 与 Transformation（默认最新 enabled 版本，版本 Tag 回显）；Data Latency 来自 Feature Source（Tag）；Source Type / Input Params 自动展示；可无块（仅离线 Training）。
  - **Feature Mapping**：Serving 输出列表映射到 Training 表字段（可选、支持 Auto Match）；下方可增删 **Compute feature**（名称、SQL、类型），SQL 中引用的特征名须属于本步 Serving 输出集合。
