...
- **唯一真源（实现）**：[../../prototypes/feature-widetable/](../../prototypes/feature-widetable/)（Vite + React）。
- **旧版单文件归档**：[FEATURE_STORE.legacy.html](./FEATURE_STORE.legacy.html)（历史参考）。

### 本页「打开演示」链接（FEATURE_STORE.html）

`dist/` 默认不提交仓库。在 `prototypes/feature-widetable/` 执行 `**pnpm run sync-prototype`**，会把构建结果复制到 `**docs/prototype/feature-store-demo/`**，与 `FEATURE_STORE.html` 同级，便于 `file://` 下点击链接或双击 `index.html`。

`feature-store-demo/` 已列入 `[./.gitignore](./.gitignore)`，需在本机生成。

### 分享给他人（无公网 URL 时）

1. 在 `prototypes/feature-widetable/` 执行 `pnpm install` 与 `pnpm run build`（或 `pnpm run sync-prototype`）。
