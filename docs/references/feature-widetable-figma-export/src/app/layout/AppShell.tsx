import { NavLink, Outlet } from "react-router";

const NAV_ITEMS: { to: string; label: string; end?: boolean }[] = [
  { to: "/fs", label: "Feature Source", end: true },
  { to: "/tf", label: "Transformation", end: true },
  { to: "/fg", label: "Feature Group", end: false },
  { to: "/fm", label: "Feature Map", end: true },
  { to: "/wt", label: "WideTable", end: false },
];

export function AppShell() {
  return (
    <div className="flex h-screen overflow-hidden bg-[#f0f2f5] text-[13px] text-gray-800">
      <aside className="w-64 min-w-[256px] shrink-0 flex flex-col border-r border-gray-200/80 bg-[#f0f2f5]">
        <div className="h-12 flex items-center gap-2 px-4 border-b border-black/[0.06]">
          <div className="w-6 h-6 rounded-md bg-teal-500 text-white text-xs font-bold flex items-center justify-center">
            F
          </div>
          <span className="text-sm font-semibold text-gray-800 truncate">
            Online FeatureStore
          </span>
        </div>
        <nav className="flex-1 py-2 overflow-y-auto">
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              end={item.end}
              className={({ isActive }) =>
                [
                  "flex items-center gap-2.5 px-4 py-2.5 cursor-pointer border-l-[3px] transition-colors",
                  isActive
                    ? "bg-teal-500 text-white border-teal-500"
                    : "border-transparent text-gray-600 hover:bg-black/[0.04] hover:text-gray-900",
                ].join(" ")
              }
            >
              <span className="w-5 text-center text-[15px] opacity-90">◇</span>
              <span className="whitespace-nowrap overflow-hidden text-ellipsis">
                {item.label}
              </span>
            </NavLink>
          ))}
        </nav>
      </aside>

      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-12 shrink-0 bg-white border-b border-gray-200 flex items-center px-6 text-sm font-medium text-gray-800">
          Feature Platform
        </header>
        <div className="flex-1 overflow-y-auto min-h-0">
          <Outlet />
        </div>
      </div>
    </div>
  );
}
